...
class ArticleSemanticAnalyzer:
    ...
    def _extract_effects(self, text: str) -> list[dict[str, Any]]:
        ...
        has_damages_terms = self._has_any(lowered, ("возместить убыт", "возмещения убыт", "возмещение убыт", "убытки возмещ", "взысканы убыт", "отвечает перед кредитором за убытки")) or (
